...
func AllAlbumInfos(db *sql.DB) ([]m.AlbumInfos, error) {
	...
	rows, err := db.Query("SELECT a.name, (SELECT m.media_id FROM media_to_album m WHERE m.album = a.name LIMIT 1), COUNT(ma.album) FROM public.album a, public.media_to_album ma WHERE a.name = ma.album GROUP BY a.name")
